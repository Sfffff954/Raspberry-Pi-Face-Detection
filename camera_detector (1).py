...
MQTT_CONTROL_TOPIC  = "home/security/control"  # NEW: listens for arm/disarm
...
# Global arm state (can be toggled via MQTT)
system_armed = True
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        log.info("Subscribed to control topic: %s", MQTT_CONTROL_TOPIC)
    else:
        log.error("MQTT connection failed, rc=%d", rc)

def on_message(client, userdata, msg):
    """Handle arm/disarm commands from face_recognizer or Telegram."""
    global system_armed
    try:
        ...
        if cmd == "arm":
            system_armed = True
            log.info("System ARMED via MQTT.")
        elif cmd == "disarm":
            system_armed = False
            log.info("System DISARMED via MQTT.")
    except Exception as e:
        log.warning("Could not parse control message: %s", e)
...
def main():
    ...
    client.on_message = on_message          # NEW: handle control messages
    ...
    try:
        while True:
            # If disarmed, skip all detection
            if not system_armed:
                time.sleep(0.5)
                continue
